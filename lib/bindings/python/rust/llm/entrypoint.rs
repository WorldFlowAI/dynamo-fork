    ...
    let inner = match args.engine_type {
        ...
        EngineType::Mocker => {
            ...
            if let Some(ref backend_name) = mocker_args.aic_backend {
                ...
                let system = mocker_args
                    .aic_system
                    .as_deref()
                    .unwrap_or("h200_sxm");
                let model_name = local_model.display_name();
                let backend_version = mocker_args.aic_backend_version.as_deref();
                match Python::with_gil(|py| {
                    create_aic_callback(py, &backend, system, model_name, 1, backend_version)
                }) {
                    Ok(callback) => {
                        tracing::info!(
                            "AIC perf model: backend={}, gpu={}, model={}, version={:?}",
                            backend, system, model_name, backend_version
                        );
