...
impl PerfModel {
    ...
    /// - Polynomial/Interpolated: uses `isl - prefix` (new tokens only)
    /// - Aiconfigurator: passes (batch_size, isl, prefix) to the Python AIC SDK
    pub fn predict_prefill_time(&self, batch_size: usize, isl: usize, prefix: usize) -> f64 {
        ...
        let time = match self {
            ...
            PerfModel::Interpolated { prefill_interp, .. } => {
                prefill_interp.interp(new_tokens as f64).unwrap_or(0.0)
            }
            PerfModel::Aiconfigurator { callback } => {
